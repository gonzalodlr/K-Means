...
public class ClusterSet {
	...
	public void initializeCentroids(Data data) throws OutOfRangeSampleSize {
		...
		for (int i = 0; i < centroidIndexes.length; i++) {
			System.out.println("Element index:" + centroidIndexes[i]);
			Tuple centroid = data.getItemSet(centroidIndexes[i]);
			if (((String) centroid.get(0).getValue()).equals("overcast")) {
				double uno = (double) centroid.get(1).getValue();
				System.out.println("overcast " + uno + " " + (String) centroid.get(2).getValue() + " "
						+ (String) centroid.get(3).getValue() + " " + (String) centroid.get(4).getValue());

			} else {
				System.out.println((String) centroid.get(0).getValue() + " " + (double) centroid.get(1).getValue() + " " + (String) centroid.get(2).getValue() + " " + (String) centroid.get(3).getValue() + " " + (String) centroid.get(4).getValue());
			}
